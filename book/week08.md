...
1) 내부 클래스의 종류를 말하세요
1) 인스턴스 내부 클래스/ 정적 내부 클래스/ 지역 내부 클래스/ 익명 클래스
...
2) 정적 내부 클래스에서는 외부 전역 변수만을 사용할 수 있다?
2) O
```java
class OuterClass {
    private String str1 = "외부 클래스 인스턴스 변수입니다.";
...
3) 익명 내부 클래스의 대표적인 특징은?
3) 클래스의 선언과 객체의 생성을 동시에 할 수 있는 일회용 익명 클래스
...
4) 익명 클래스는 여러개의 인터페이스를 구현할 수 있을까?
4) X
